...
def run_pseudobulk_pydeseq2_clean():
    # Paths relative to project root
    input_path = 'scAR_python_validation/data/v20_python_gold_standard.h5ad'
    output_dir = 'scAR_python_validation_v2_clean/results/pydeseq2'
    ...
    # Exclusion patterns based on reference notebook
    ...
    print(f"   Genes identified for removal: {len(removed_genes)}")
    if len(removed_genes) > 0:
        print(f"   Examples: {removed_genes[:5]}")
    ...
    pb.write_h5ad(f"{output_dir}/v20_clean_pseudobulk_aggregated.h5ad", compression='gzip')
    
    # 4. PyDESeq2 Analysis
    print("\n🚀 Running PyDESeq2 Analysis (Clean Iteration)...")
